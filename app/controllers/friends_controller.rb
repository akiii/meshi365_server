class FriendsController < ApplicationController
  ...
  def send_friend_request
    ...
    if me != nil && request_user != nil && params[:my_uiid] != params[:to_user_uiid]
      req = FriendRequest.find_by_my_id_and_from_user_id(me.id, request_user.id)
      if req
        ...
        unless FriendRequest.exists?(:my_id => request_user.id, :from_user_id => me.id)
          FriendRequest.create(:my_id => request_user.id, :from_user_id => me.id)
        end
      end
    end
  end
